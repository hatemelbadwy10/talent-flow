...
class _ProjectCardState extends State<ProjectCard> {
  ...
  void didUpdateWidget(covariant ProjectCard oldWidget) {
    ...
    if (oldWidget.projectsModel != widget.projectsModel) {
      _currentProjectModel = widget.projectsModel;
    }
  }
  ...
  Widget build(BuildContext context) {
                ...
                onPressed: () async {
                  if (_currentProjectModel.id != null) {
                    ...
                    if (newProjectsBloc != null) {
                      newProjectsBloc.add(
                        ProjectFavoriteToggled(_currentProjectModel.id!),
                      );
                      return;
                    }
          ...
          Row(
            children: [
              _buildMetaInfo(
                ...
                text: _currentProjectModel.since ??
                    "project_card.posted_ago".tr(),
              ),
    ...
    return BlocListener<NewProjectsBloc, NewProjectsState>(
      listener: (context, state) {
        if (state is ProjectFavoriteFailed &&
            state.projectId == _currentProjectModel.id) {
          setState(() {
            _currentProjectModel = _currentProjectModel.copyWith(
